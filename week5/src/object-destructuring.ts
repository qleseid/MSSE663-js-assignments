...
// reconstruct here
export const newOb = {user, address, id};
// expected results:
console.log(newOb.user); // 'elvis'
console.log(newOb.address); // { city: 'denver', state: 'colorado' }
console.log(newOb.id); // 1

// #4 Object with key value pairs: construct a statement
export const three = { greeting: 'hello', name: 'taylor' };

// construct statement here
export function cons ({greeting="", name=""} = {}): string
{
  return `${greeting}, ${name}!`;
}
// expected result: 'hello, taylor!'
console.log(cons(three));